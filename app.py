...
# --- Signal helpers ---
def save_signal(user_id, signal_time, pair, direction):
    db = get_db()
    db.execute('INSERT INTO signals (user_id, pair, direction, confidence, created_at) VALUES (?, ?, ?, ?, ?)',
        ...
        for source_func in data_sources:
            try:
                ...
                if price is not None:
                    ...
                    if return_source:
                        return price, 'Alpha Vantage' if source_func == data_sources[0] else 'ExchangeRate-API' if source_func == data_sources[1] else 'Fixer.io'
                    return price
...
def index():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return render_template("results.html", signals=signals, current_rate=current_rate, selected_pair=selected_pair, selected_broker=selected_broker, payout=payout, call_price=call_price, put_price=put_price, volatility=volatility, expiry=expiry, risk_free_rate=risk_free_rate, data_source=data_source)
        except ValueError:
            ...
            while current < end:
                ...
                if 'otc_indicators' in session:
                    ...
                    if signal_confidence >= 2:
                        direction = "CALL"
                        confidence = min(85 + (signal_confidence - 2) * 5, 95)  # Scale confidence from 85-95%
                    elif signal_confidence <= -2:
                        direction = "PUT"
                        confidence = min(85 + (abs(signal_confidence) - 2) * 5, 95)  # Scale confidence from 85-95%
                    else:
                        # If confidence is low, use the specified signal type
                        direction = signal_type if signal_type != "BOTH" else random.choice(["CALL", "PUT"])
                        confidence = 65  # Base confidence for low-confidence signals

                    # Calculate exit time (5 minutes after entry)
                    exit_time = (current + timedelta(minutes=5)).strftime("%H:%M")
                    ...
                    signals.append({
                        ...
                        "pair": pair,
                        "direction": direction,
                        "confidence": 65,  # Base confidence for random signals
                        "indicators": None
                    })
